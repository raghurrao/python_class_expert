-- 30-day SQL capstone final report.
-- Run with: python run_sql.py queries/day-30.sql
...
-- Customer activity and rank: one row per customer.
...
-- Book performance: one row per book, including books with no orders.
...
-- Reconcile the revenue across independent groupings.
WITH monthly AS (
    ...
    FROM books AS b
    INNER JOIN book_orders AS o ON b.book_id = o.book_id
    GROUP BY b.book_id
), author AS (
